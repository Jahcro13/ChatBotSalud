...
epochs = 600
#Encoder
encoder_inputs = tensorflow.keras.layers.Input(shape=(None, num_encoder_tokens))
encoder_lstm = tensorflow.keras.layers.LSTM(dimensionality, return_state=True)
encoder_outputs, state_hidden, state_cell = encoder_lstm(encoder_inputs)
encoder_states = [state_hidden, state_cell]

#Decoder
decoder_inputs = tensorflow.keras.layers.Input(shape=(None, num_decoder_tokens))
decoder_lstm = tensorflow.keras.layers.LSTM(dimensionality, return_sequences=True, return_state=True)
            ...
            for messaging_event in entry['messaging']:
                ...
                if messaging_event.get('message'):  # alguien envia un mensaje
                    ...
                    if message_text == "Hola":
                        send_message(sender_id, "Hola, escribe una pregunta")

                    else:

                        def string_to_matrix(pregunta):
                            tokens = re.findall(r"[\w']+|[^\s\w]", pregunta)
                        ...
                        def generate_response(pregunta):
                            input_matrix = string_to_matrix(pregunta)
